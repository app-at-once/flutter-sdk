...
patch_file() {
    ...
    if [ -f "$file" ]; then
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            ...
            sed -i '' 's|http://localhost:8091|https://api.appatonce.com|g' "$file"
            sed -i '' 's|https://localhost:8091|https://api.appatonce.com|g' "$file"
            sed -i '' 's|ws://localhost:8091|wss://api.appatonce.com|g' "$file"
            sed -i '' 's|wss://localhost:8091|wss://api.appatonce.com|g' "$file"
            sed -i '' "s|'localhost:8091'|'api.appatonce.com'|g" "$file"
            sed -i '' 's|"localhost:8091"|"api.appatonce.com"|g' "$file"
        else
            # Linux
            # Replace various localhost patterns
